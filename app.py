...
# --- FUNKCJE LOGICZNE (ANALIZA MEDYCZNA) ---
...
if not df_all.empty:
    ...
    if 'Mood' not in df_all.columns: df_all['Mood'] = None
...
if not df_u.empty:
    ...
    with m2:
        st.metric("CIŚNIENIE (SYS/DIA)", f"{int(safe_val(last.get('Cisnienie_S')))}/{int(safe_val(last.get('Cisnienie_D')))}")
        st.markdown(f"Status: **Stabilny**")
    with m3:
        st.metric("ŚREDNIE TĘTNICZE (MAP)", f"{map_val} mmHg")
        map_status = "Norma" if 70 <= map_val <= 100 else "Poza normą"
    ...
    with m4:
        ...
        st.markdown(f"<span class='{pp_class}'>{'Optymalne' if pp_val <= 60 else 'Podwyższone'}</span>", unsafe_allow_html=True)

    # --- ZAKŁADKI ANALITYCZNE ---
    st.markdown("<br><br>", unsafe_allow_html=True)
    tab1, tab2, tab3 = st.tabs(["📊 KARDIOLOGIA & TRENDY", "🩺 RAPORT KLINICZNY", "✨ RYTUAŁY"])

    with tab1:
        st.markdown("#### Analiza MAP (Mean Arterial Pressure) i PP (Pulse Pressure)")
        
        
        fig_adv = go.Figure()
        # Obliczamy serie danych dla wykresu
        ...
        fig_adv.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=450, 
                             legend=dict(orientation="h", y=1.1, x=0.5, xanchor="center"),
                             yaxis=dict(gridcolor='rgba(0,0,0,0.05)', title="mmHg"))
        ...
        with c_inf1:
            ...
            <div class='analysis-card'>
                <b>Co to jest MAP?</b><br>
                Średnie ciśnienie tętnicze (MAP) to średnie ciśnienie w Twoich tętnicach podczas jednego cyklu serca. 
                Norma kliniczna to zazwyczaj <b>70-100 mmHg</b>.
            </div>
            """, unsafe_allow_html=True)
        with c_inf2:
            st.markdown(f"""
            <div class='analysis-card'>
                <b>Dlaczego PP jest ważne?</b><br>
                Ciśnienie tętna (PP) to różnica między SYS a DIA. Jeśli stale przekracza 60 mmHg, 
                może to świadczyć o sztywnieniu tętnic. Twoje ostatnie PP: <b>{int(pp_val)} mmHg</b>.
            </div>
            """, unsafe_allow_html=True)

    with tab2:
        st.markdown("### Zestawienie dla Lekarza")
        col_rep1, col_rep2 = st.columns([2, 1])
        with col_rep1:
            st.table(df_u[['Data', 'Cisnienie_S', 'Cisnienie_D', 'Tetno', 'Waga', 'Dawka']].sort_values("Data", ascending=False).head(15))
        with col_rep2:
            st.markdown("<div class='report-card'>", unsafe_allow_html=True)
            st.write("**Statystyki 14 dni:**")
            st.write(f"- Średnie SYS: {int(df_u['Cisnienie_S'].tail(14).mean())}")
            st.write(f"- Średnie DIA: {int(df_u['Cisnienie_D'].tail(14).mean())}")
            st.write(f"- Średnia Waga: {df_u['Waga'].tail(14).mean():.1f} kg")
            st.markdown("</div>", unsafe_allow_html=True)
            
            csv = df_u.to_csv(index=False).encode('utf-8')
            st.download_button("📥 EKSPORTUJ CSV", csv, f"sanctuary_{user}.csv", "text/csv", use_container_width=True)

    with tab3:
        st.info("🧘 Twoje Sanctuary to miejsce spokoju. Skorzystaj z poniższych rytuałów przed pomiarem.")
        st.write("1. **Cisza:** Odczekaj 5 minut w pozycji siedzącej przed założeniem mankietu.")
        st.write("2. **Oddech:** Wykonaj 4 głębokie wdechy nosem i długie wydechy ustami.")
        st.write("3. **Woda:** Wypij szklankę wody o temperaturze pokojowej.")

# --- STOPKA Z AKCJAMI (FULL FORM) ---
st.markdown("<br><br>", unsafe_allow_html=True)
b1, b2, b3 = st.columns(3)

with b1:
    with st.popover("🧘 NOWY POMIAR", use_container_width=True):
        with st.form("add_full_form"):
            d = st.date_input("Data", datetime.now())
            w = st.number_input("Waga (kg)", step=0.1, value=safe_val(df_u['Waga'].iloc[-1]) if not df_u.empty else 80.0)
            c1, c2, c3 = st.columns(3)
            s_v = c1.number_input("SYS", value=120)
            d_v = c2.number_input("DIA", value=80)
            p_v = c3.number_input("Puls", value=70)
            mood_v = st.selectbox("Nastrój", ["🌿 Spokój", "☀️ Energia", "☁️ Zmęczenie", "⚡ Stres", "🌊 Równowaga"])
            ds_v = st.selectbox("Dawka (mg)", [0.0, 0.25, 0.5, 0.75, 1.0, 1.5, 2.0])
            if st.form_submit_button("ZAPISZ W ARKUSZU"):
...
with b2:
    with st.popover("✨ KOREKTA DANYCH", use_container_width=True):
        if not df_u.empty:
            sel_date = st.selectbox("Wybierz datę do zmiany:", df_u['Data'].tolist()[::-1])
...
st.markdown("<div style='text-align: center; margin-top: 50px; opacity: 0.3; font-size: 0.7rem; letter-spacing: 3px;'>THE SANCTUARY • BIOMETRYCZNA HARMONIA • 2026</div>", unsafe_allow_html=True)
